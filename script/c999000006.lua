...
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_DOGS) and c:HasLevel()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT)
		...
		and g:GetClassCount(Card.GetLevel)>=1 end
	local sg=aux.SelectUnselectGroup(g,e,tp,1,1,aux.dpcheck(Card.GetLevel),1,tp,HINTMSG_SPSUMMON)
